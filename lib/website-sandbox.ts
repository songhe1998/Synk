...
async function rewriteStaticPreviewAssetPaths(sandbox: Sandbox) {
  const script = [
    ...
    "  if (isJs) {",
    "    text = text",
    "      .replace(/((?:src|href|action)=[\"'])\\/(?=(images|icon|apple-icon|placeholder))/g, '$1./')",
    "      .replace(/url\\(\\s*\\/(?=(images|icon|apple-icon|placeholder))/g, 'url(./')",
    "      .replace(/([\"'`])\\/(?=(images|icon|apple-icon|placeholder))/g, '$1./')",
    "      .replace(/(\\\\\")\\/(?=(images|icon|apple-icon|placeholder))/g, '$1./');",
    "  } else {",
    "    text = text",
