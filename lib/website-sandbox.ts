...
async function rewriteStaticPreviewAssetPaths(sandbox: Sandbox) {
  const script = [
    ...
    "  if (isJs) {",
    "    text = text",
    "      .replace(/((?:src|href|action)=[\"'])\\/(?=(images|icon|apple-icon|placeholder|v0-assets))/g, '$1./')",
    "      .replace(/url\\(\\s*\\/(?=(images|icon|apple-icon|placeholder|v0-assets))/g, 'url(./')",
    "      .replace(/([\"'`])\\/(?=(images|icon|apple-icon|placeholder|v0-assets))/g, '$1./')",
    "      .replace(/(\\\\\")\\/(?=(images|icon|apple-icon|placeholder|v0-assets))/g, '$1./');",
    "  } else {",
    "    text = text",
    "      .replace(/((?:src|href|action)=[\"'])\\/(?=(_next|images|icon|apple-icon|placeholder|v0-assets))/g, '$1./')",
    "      .replace(/url\\(\\s*\\/(?=(_next|images|icon|apple-icon|placeholder|v0-assets))/g, 'url(./')",
    "      .replace(/([\"'`])\\/(?=(_next|images|icon|apple-icon|placeholder|v0-assets))/g, '$1./')",
    "      .replace(/(\\\\\")\\/(?=(_next|images|icon|apple-icon|placeholder|v0-assets))/g, '$1./');",
    "  }",
